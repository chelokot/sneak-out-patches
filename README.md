...
| `chair-wall-throw-fix` | Chair Wall Throw Fix | Yes | Removes availability, distance and overlap rejection from the explicit Throw path for every in-hand chair/stool, barrel and task ingredient, so the crossed-out interaction state cannot block releasing a held throwable. |

...
internal static class LocalSelectionsStore
{
    ...
    public static void SaveCharacterSelection(Character character)
    {
        lock (Sync)
        {
            UnlockEverythingRuntime.LogSkinSelectionSnapshot("LocalSelectionsStore.SaveCharacterSelection", character);
    ...
    public static void SaveCharacterSkin(CharacterType characterType, CharacterSkin characterSkin)
    {
        ...
        {
            var profileKey = UnlockEverythingStub.GetProfileStorageKey();
            UnlockEverythingRuntime.LogPersistentSelectionStore("LocalSelectionsStore.SaveCharacterSkin:key", profileKey, characterType);
            var profileSelections = GetOrCreateProfileSelections(profileKey);
    ...
    public static void SaveSkinPartSelection(CharacterType characterType, SkinType skinType, SkinPartType skinPartType)
    {
        ...
        {
            var profileKey = UnlockEverythingStub.GetProfileStorageKey();
            UnlockEverythingRuntime.LogPersistentSelectionStore("LocalSelectionsStore.SaveSkinPartSelection:key", profileKey, characterType, skinType, skinPartType);
            var profileSelections = GetOrCreateProfileSelections(profileKey);
            if (!profileSelections.Characters.TryGetValue(((int)characterType).ToString(), out var selection))
            {
                selection = new PersistedCharacterSelection();
    ...
    public static void ApplySelections(WebPlayer player)
    {
        lock (Sync)
        {
            ...
            foreach (var character in player.Characters ?? new Il2CppCollections.List<Character>())
            {
                ...
                UnlockEverythingRuntime.LogPersistentSelectionLoad("LocalSelectionsStore.ApplySelections", character, selection);
    ...
    private static Avatar FindOrCreateAvatar(WebPlayer player, AvatarType avatarType)
    {
        ...
        var createdAvatar = new Avatar(UnlockEverythingStub.GetAvatarId(avatarType), avatarType);
    ...
    private static AvatarFrame FindOrCreateAvatarFrame(WebPlayer player, AvatarFrameType avatarFrameType)
    {
        ...
        var createdFrame = new AvatarFrame(UnlockEverythingStub.GetAvatarFrameId(avatarFrameType), avatarFrameType);
    ...
    private static Emote FindOrCreateEmote(WebPlayer player, EmoteType emoteType)
    {
        ...
        var createdEmote = new Emote(UnlockEverythingStub.GetEmoteId(emoteType), emoteType);
    ...
    private static SkinPart FindOrCreateSkinPart(WebPlayer player, SkinType skinType, SkinPartType skinPartType)
    {
        ...
        var createdSkinPart = new SkinPart(UnlockEverythingStub.GetSkinPartId(skinPartType), skinType, skinPartType);
    ...
    private static SkillCard FindOrCreateSkillCard(WebPlayer player, SkillType skillType)
    {
        ...
        {
            return UnlockEverythingStub.EnsureMaxSkillCard(existingCard, skillType);
        }

        var createdCard = UnlockEverythingStub.CreateMaxSkillCard(skillType);
    ...
    private static void EnsurePlayerCharacterSkin(WebPlayer player, CharacterSkin characterSkin)
    {
        ...
        player.CharacterSkins.Skins.Add(new PlayerCharacterSkin(UnlockEverythingStub.GetCharacterSkinId(characterSkin), characterSkin));
    }
    ...
    private static PersistedProfileSelections? GetExistingProfileSelections()
    {
        ...
        var primaryProfileKey = UnlockEverythingStub.GetProfileStorageKey();
        if (_root.Profiles.TryGetValue(primaryProfileKey, out var primarySelections))
        {
            return primarySelections;
        }

        var legacyProfileKey = UnlockEverythingStub.GetLegacyProfileStorageKey();
    ...
    private static void Save()
    {
        ...
        {
            UnlockEverythingRuntime.LogError("Backend stabilizer failed to save persistent selections", exception);
        }
    }
}

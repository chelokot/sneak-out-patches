...
| `minimap` | Minimap | No | Generates a lightweight camera-aligned floor plan with rounded rooms, doors, pass-through doorway frames, teleport wardrobes, and item rollers from loaded runtime geometry. It shows only the local player and adds a stock-styled Map tab for shape, zoom, placement, visibility, and key binding. It uses no second render camera. |
...
| `lobby-test-bot` | Lobby Test Bot | No | Adds host-only portal controls that spawn or remove one inert real Fusion bot and choose Penguin or Hunter Priority for Classic private tests. A hunter bot automatically confirms the stock Ripper so selection cannot stall. It is excluded from compatibility and host-selection counts. |

...
| `chair-wall-throw-fix` | Chair Wall Throw Fix | Yes | Lets an already held chair/stool, barrel or task ingredient be released beside walls: bypasses the stock forward-ray `None` result, hides its crossed-out `E` overlay, accepts the explicit Throw request and corrects a chair's overlapping release position without disabling the shared obstacle detector. |
